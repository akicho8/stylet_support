...
class Scene
  ...
  def update
    ...
    @vA = Stylet::Vector.sincos(Stylet::Fee.clock(@hour)).scale(@win.rect.hy * 0.8)    # ベクトルAはキーボードで移動できる
    @vB = Stylet::Vector.sincos(@win.rect.center.angle_to(@win.cursor)).scale(@win.rect.hy * 0.8)  # ベクトルBはマウスの方を向く
