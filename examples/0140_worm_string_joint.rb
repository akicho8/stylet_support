  ...
  def update
    ...
    if @target
      ...
      if gap > 0
        ...
        dir = @target.p0.rdirf(@p0)
        @target.p0.x += Stylet::Fee.rcosf(dir) * len
        @target.p0.y += Stylet::Fee.rsinf(dir) * len

        # 固さ 1.0 のときは次のように p0 の方から相手をひっぱる方法でもよいが前者の方が、ゆっくり移動させるなど応用が効く
        # dir = @p0.rdirf(@target.p0)
        # @target.p0.x = @p0.x + Stylet::Fee.rcosf(dir) * @base.length
        # @target.p0.y = @p0.y + Stylet::Fee.rsinf(dir) * @base.length
      end
    end
